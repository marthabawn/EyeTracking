...
for f in glob.glob("TransInf_Behav_Data/LSAT_T2/*behavioralout.txt"):
...
df = df.sort(['PID', 'Trial'])  # sort by PID and Trial
df = df.reset_index()
df = df.drop('index', axis=1)

### Create dataframe gaze_stats from gaze data
gaze_stats = pd.read_csv('Gaze_Stats/LSAT_T2transinf_gaze_statistics.txt', delimiter='\t', header=None)
...
new_df.to_csv('All_Stats/LSAT_T2transinf_behavioral_gaze_statistics.csv')
