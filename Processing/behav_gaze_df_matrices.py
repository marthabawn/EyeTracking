...
### Combines Ogama's gaze data with the raw behavioral data

# Create dictionary that maps trial number to condition
...
#It's easier to separate the files from subject 1-10 into a separate folder
#Reads all the _behavioral files in the folder specified
for f in glob.glob("Matrices_Behav_Data/LSAT_T2/*behavioralout.txt"):
    ...
    with open(f, 'r') as csvfile:
        ...
        # Add columns for PID, Group, Block, Accuracy, and Condition
...
gaze_stats = pd.read_csv('Gaze_Stats/LSAT_T2matrices_gaze_statistics.txt', delimiter='\t', header=None)
gaze_stats.columns = ['PID', 'Trial', 'Duration', 'Fix/SaccadeRatio',
...
column_order = ['PID', 'Block', 'Trial', 'Condition', 'CorrectAnswer', 'SolutionClicked', 'SubjectResponse',
...
new_df.to_csv('All_Stats/LSAT_T2matrices_behavioral_gaze_statistics.csv')
